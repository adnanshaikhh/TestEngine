...
void Camera::Update() 
{
	...
	if (camera_mode == ORTHO) {
		...
		camera_heading *= .5;
		camera_pitch *= .5;
		//camera_position_delta = camera_position_delta * .8f; //Limits Camera Movement, Smooth as fuck tho
	}

	else if (camera_mode == ACTOR)
	{
		projection = glm::perspective(field_of_view, aspect, near_clip, far_clip);
		//detmine axis for pitch rotation
		glm::vec3 axis = glm::cross(camera_direction, camera_up);
	...
	//cout << "camera_look_at(" << camera_look_at.x << "," << camera_look_at.y << "," << camera_look_at.z << ")" << endl;
...
void Camera::Translate(const glm::vec3& delta, float speedTime)
{

	if (delta.z > 0)
	{

		camera_position_delta += camera_direction * camera_scale *speedTime; //Forward
	
	}
	if (delta.z < 0)
	{
		camera_position_delta -= camera_direction * camera_scale * speedTime; //backward
	}

	if (delta.x>0)
	{
		camera_position_delta += glm::cross(camera_direction, camera_up) * camera_scale * speedTime; //Right
	}
	if (delta.x < 0)
	{
		camera_position_delta -= glm::cross(camera_direction, camera_up) * camera_scale * speedTime; //Left
	}

	if (delta.y>0)
	{
		camera_position_delta += camera_up * camera_scale * speedTime; //Upward
	}
	if (delta.y < 0)
	{
		camera_position_delta -= camera_up * camera_scale * speedTime; //Downward
	}
...
void Camera::Move2D(int x, int y) {
	...
	//std::cout << "dX: " << mouse_delta.x << " dy: " << mouse_delta.y << std::endl;

	//if the camera is moving, meaning that the mouse was clicked and dragged, change the pitch and heading
	if (move_camera) {
		ChangeHeading(.08333f * mouse_delta.x);
		ChangePitch(.08333f * mouse_delta.y);
	}
